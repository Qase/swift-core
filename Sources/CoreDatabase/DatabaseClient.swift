...
extension DatabaseClient: DatabaseClientType {
  ...
  private func load<DatabaseModel: NSManagedObject & Identifiable>(byID modelID: AnyHashable) -> Result<DatabaseModel?, DatabaseError> {
    let fetchRequest: NSFetchRequest<DatabaseModel> = NSFetchRequest(entityName: DatabaseModel.entityName)
    fetchRequest.predicate = NSPredicate.single(withID: modelID)
  ...
  public func observe<DatabaseModel: NSManagedObject & Identifiable, DomainModel: Identifiable & Equatable>(
    ...
    return context.fetchPublisher(specifiedBy: fetchRequest)
...
private extension NSPredicate {
  static func single(withID modelID: AnyHashable) -> NSPredicate {
    NSPredicate(format: "%K == %@", "id", modelID as CVarArg)
  }
}
