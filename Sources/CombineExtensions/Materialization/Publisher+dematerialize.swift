...
public extension Publishers.Dematerialize {
...
private extension Publishers.Dematerialize {
  ...
  where Downstream.Input == Upstream.Output.Output, Downstream.Failure == Upstream.Output.Failure {
    private var sink: DematerializeSink<Downstream>?

    init(
      upstream: Upstream,
      downstream: Downstream
    ) {
      self.sink = DematerializeSink(
        upstream: upstream,
        downstream: downstream
      )
    }
...
// MARK: - Sink
private extension Publishers.Dematerialize {
  class DematerializeSink<Downstream: Subscriber>: Sink<Upstream, Downstream>
  where Downstream.Input == Upstream.Output.Output, Downstream.Failure == Upstream.Output.Failure {
