...
class Publisher_retryWhenTests: XCTestCase {
  ...
  func test_retryWhen_multiple_events_cause_multiple_retries() {
    ...
    let source = Publishers.Create<String, TestError> { subscriber in
      let emit = {
        defer { emitCount += 1 }

        if emitCount == 2 {
          subscriber.send("Success!")
          subscriber.send(completion: .finished)
        } else {
          subscriber.send(completion: .failure(.errorNum1))
        }
      }
    ...
    source
      ...
      .retryWhen { errorSignal -> AnyPublisher<Void, TestError> in
        errorSignal
          .flatMap { _ -> AnyPublisher<Void, TestError> in
            return Publishers.Create<Void, TestError> { subscriber in
              subscriber.send(())
              subscriber.send(())
              subscriber.send(completion: .finished)
